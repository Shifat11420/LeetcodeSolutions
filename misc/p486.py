...
class Solution:
    ...
    #         scoreLeft = nums[left]-maxDiff(left+1, right)
    #         scoreRight = nums[right]-maxDiff(left, right-1)
    #         cache[(left, right)] = max(scoreLeft, scoreRight)
    #         return cache[(left, right)]    
    #     maxDiff(0, len(nums)-1)
    #     return cache[(0, len(nums)-1)]>=0             
    ...
    # DP-space optimized, time O(n**2), Space O(n)
    def predictTheWinner(self, nums: List[int]) -> bool:
        n = len(nums)
        dp = nums[:]
        ...
        for diff in range(1, n):
            ...
            for left in range(n-diff):
                right = left+diff
                nextdp[left] = max(nums[left]-dp[left+1], nums[right]-dp[left])
            dp = nextdp
        return dp[0]>=0
